/**
 * @file wifi_tool.h
 * @brief MCP-Inspired WiFi Tool Interface
 * 
 * Transformed from wifi_manager to follow MCP tool composition patterns.
 * Provides handle-based WiFi management with event-driven communication.
 */
...
#define WIFI_TOOL_ID            "wifi"
#define WIFI_TOOL_VERSION       "2.0.0"
#define WIFI_TOOL_DESCRIPTION   "MCP-inspired WiFi connectivity tool with multi-network support"

#define WIFI_TOOL_MAX_NETWORKS  5
#define WIFI_TOOL_MAX_SSID_LEN  32
#define WIFI_TOOL_MAX_PASS_LEN  64

// =============================================================================
// MCP Tool Events System
// =============================================================================

ESP_EVENT_DECLARE_BASE(WIFI_TOOL_EVENTS);

/**
 * @brief WiFi Tool Event Types (Published for other tools)
 */
typedef enum {
    WIFI_TOOL_EVENT_STA_CONNECTING = 0,     ///< Attempting to connect to network
    ...
    WIFI_TOOL_EVENT_AP_STARTED,             ///< AP mode started and ready
    ...
    WIFI_TOOL_EVENT_CONFIG_CHANGED,         ///< Network configuration updated
    WIFI_TOOL_EVENT_IP_ACQUIRED,            ///< Got IP address (STA mode)
    WIFI_TOOL_EVENT_IP_LOST,                ///< Lost IP address
} wifi_tool_event_type_t;

/**
 * @brief WiFi Tool Event Data Structure
 */
typedef struct {
    wifi_tool_event_type_t type;
    union {
        struct {
            char ssid[WIFI_TOOL_MAX_SSID_LEN];
            uint8_t bssid[6];
            int8_t rssi;
        } sta_info;
        struct {
        ...
        struct {
            char ap_ssid[WIFI_TOOL_MAX_SSID_LEN];
 ...
 */
typedef enum {
    WIFI_CAP_STA_MODE        = (1 << 0),    ///< Station mode support
    WIFI_CAP_AP_MODE         = (1 << 1),    ///< Access Point mode support
    WIFI_CAP_MULTI_NETWORK   = (1 << 2),    ///< Multiple network configuration
    WIFI_CAP_AUTO_RECONNECT  = (1 << 3),    ///< Automatic reconnection
    WIFI_CAP_CONFIG_PERSIST  = (1 << 4),    ///< Configuration persistence
    WIFI_CAP_EVENT_PUBLISH   = (1 << 5),    ///< Event publishing to other tools
    WIFI_CAP_HEALTH_MONITOR  = (1 << 6),    ///< Connection health monitoring
    WIFI_CAP_CONCURRENT_MODE = (1 << 7),    ///< STA+AP concurrent mode
} wifi_tool_capabilities_t;
...
typedef struct {
    char ssid[WIFI_TOOL_MAX_SSID_LEN];
    char password[WIFI_TOOL_MAX_PASS_LEN];
...
typedef struct {
    ...
    wifi_network_config_t networks[WIFI_TOOL_MAX_NETWORKS];
    uint8_t network_count;
    
    // AP Mode Configuration
    wifi_tool_ap_config_t ap_config;
 ...
 */
typedef struct wifi_tool_context* wifi_tool_handle_t;
...
typedef struct {
    ...
    char current_ssid[WIFI_TOOL_MAX_SSID_LEN]; ///< Connected SSID
    ...
    wifi_tool_capabilities_t capabilities;   ///< Tool capabilities
} wifi_tool_status_t;
...
typedef struct {
    ...
    wifi_tool_handle_t (*init_func)(const wifi_tool_config_t* config);
    esp_err_t (*deinit_func)(wifi_tool_handle_t handle);
} wifi_tool_registry_t;
 ...
 * @return ESP_OK on success, error code on failure
 */
esp_err_t wifi_tool_connect(wifi_tool_handle_t handle, const char* ssid, const char* password);
 ...
 * @return ESP_OK on success, error code on failure
 */
esp_err_t wifi_tool_disconnect(wifi_tool_handle_t handle);
 ...
 * @return ESP_OK on success, error code on failure
 */
esp_err_t wifi_tool_add_network(wifi_tool_handle_t handle, const wifi_network_config_t *network);
 ...
 * @return ESP_OK on success, error code on failure
 */
esp_err_t wifi_tool_remove_network(wifi_tool_handle_t handle, const char* ssid);
 ...
 * @return ESP_OK on success, error code on failure
 */
esp_err_t wifi_tool_load_config(wifi_tool_handle_t handle, const char* file_path);
 ...
 * @return ESP_OK on success, error code on failure
 */
esp_err_t wifi_tool_save_config(wifi_tool_handle_t handle, const char* file_path);
 ...
 * @return ESP_OK on success, error code on failure
 */
esp_err_t wifi_tool_update_config(wifi_tool_handle_t handle, const wifi_tool_config_t *config);
